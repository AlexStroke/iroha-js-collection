import { EventSubscriberMessage, VersionedEventPublisherMessage, VersionedEventSubscriberMessage, BlockSubscriberMessage, VersionedBlockPublisherMessage, VersionedBlockSubscriberMessage, Enum, TransactionPayload, OptionU32, MapNameValue, VersionedTransaction, Transaction, VecSignatureOfTransactionPayload, QueryPayload, PredicateBox, Predicate, VersionedSignedQueryRequest, SignedQueryRequest, VersionedPaginatedQueryResult, QueryError, Signature, PublicKey } from '@iroha2/data-model';
...
const debug$1 = Debug('@iroha2/client:events');
/**
...
class Client {
    ...
    async submit(executable, params) {
        ...
        try {
            let finalBytes;
            scope.run(() => {
                const payloadHash = collect(createHash(TransactionPayload.toBuffer(payload)));
                const signature = makeSignature(keyPair, payloadHash.bytes());
                finalBytes = VersionedTransaction.toBuffer(VersionedTransaction('V1', Transaction({ payload, signatures: VecSignatureOfTransactionPayload([signature]) })));
            });
...
export { Client, ClientIncompleteConfigError, ResponseError, getCrypto, setCrypto, setupBlocksStream, setupEvents };
